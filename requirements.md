...
- 클릭 시 피드백 모달: 별점(1~5), 평가 텍스트, 문의사항
- 평가 항목: "사용 편의성", "기능 만족도", "전체적인 만족도"
- 개발자 연락처: 이메일 주소 표시
- 데이터는 MongoDB에 저장, 개발자가 직접 확인

### 화면 구성
- 모달에 별점 선택 UI, 텍스트 입력 필드, 전송 버튼
- 개발자 정보: "문의: your-email@example.com"
...
- 중앙: 방 이름
- 우측: 본인 세션 아이콘 + 이름 (클릭 시 프로필 수정 화면)
...
- 각 카드:
  - 좌측: 펼쳐보기 토글 버튼 (▼/▲)
  - 중앙: 곡 제목, 가수, 필요 세션 아이콘
  - 우측: 좋아요 버튼 (숫자), 불가능 버튼 (숫자)
- 펼쳐보기 시:
  - 장르, 세션별 난이도 표시
  - 유튜브 임베드 (바로 재생 가능)
  - 투표 결과: 좋아요한 사람들 (이름 + 세션 아이콘)
  - 불가능 투표한 사람들 (이름 + 세션 아이콘 + 이유)
  - 댓글 리스트 (작성자 이름 + 세션 아이콘 + 내용 + 작성일)
  - 댓글 입력 필드 + 작성 버튼
